...
function getStatusClassName(status: string) {
  ...
  if (status === "in_progress") return "bg-ulv-yellow/20 text-ulv-blue";
...
export function SupportPanel() {
  ...
  if (isLoading) return <Card><p className="text-sm font-semibold text-slate-600">Cargando soporte...</p></Card>;
  if (!isAuthenticated) return <Card className="text-center"><ShieldAlert className="mx-auto h-10 w-10 text-ulv-blue" aria-hidden="true" /><h2 className="mt-3 text-xl font-black text-ulv-blue">Debes iniciar sesión para enviar una solicitud de soporte.</h2><Link href="/login?redirect=/soporte" className="mt-5 inline-flex min-h-12 items-center justify-center rounded-2xl bg-ulv-yellow px-5 py-3 text-sm font-bold text-ulv-blue">Iniciar sesión</Link></Card>;
  ...
  return (
    <div className="space-y-5 pb-24 md:pb-0">
      ...
      {activeTab === "mine" ? <Card><div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="text-xl font-black text-ulv-blue">Mis solicitudes</h2><p className="mt-1 text-sm text-slate-600">Consulta el estado y responde a tus tickets.</p></div><button type="button" onClick={() => void loadData()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-ulv-yellow px-4 py-2 text-sm font-black text-ulv-blue"><RefreshCw className="h-4 w-4" aria-hidden="true" />Actualizar</button></div>{tickets.length === 0 ? <p className="rounded-2xl bg-slate-50 p-5 text-center text-sm font-semibold text-slate-500">No tienes solicitudes registradas.</p> : null}<div className="space-y-3 md:hidden">{tickets.map((ticket) => <article key={ticket.id} className="rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-start justify-between gap-3"><h3 className="text-base font-black text-ulv-blue">{ticket.subject}</h3><span className={`rounded-full px-3 py-1 text-xs font-black ${getStatusClassName(ticket.status)}`}>{supportStatusLabels[ticket.status]}</span></div><p className="mt-2 text-sm text-slate-600">{ticket.libraries?.name ?? "General"} · {supportCategoryLabels[ticket.category]} · {supportPriorityLabels[ticket.priority]}</p><p className="mt-2 text-xs font-semibold text-slate-500">{formatDate(ticket.created_at)}</p><button type="button" onClick={() => void openTicket(ticket)} className="mt-4 min-h-11 w-full rounded-xl bg-ulv-yellow px-3 py-2 text-sm font-black text-ulv-blue">Ver detalle</button></article>)}</div><div className="hidden overflow-x-auto rounded-2xl border border-slate-200 md:block"><table className="min-w-[900px] w-full text-left text-sm"><thead className="bg-ulv-blue text-white"><tr><th className="px-4 py-3">Asunto</th><th className="px-4 py-3">Biblioteca</th><th className="px-4 py-3">Categoría</th><th className="px-4 py-3">Prioridad</th><th className="px-4 py-3">Estado</th><th className="px-4 py-3">Fecha</th><th className="px-4 py-3">Última actualización</th><th className="px-4 py-3">Acciones</th></tr></thead><tbody className="divide-y divide-slate-200 bg-white">{tickets.map((ticket) => <tr key={ticket.id}><td className="px-4 py-3 font-black text-ulv-blue">{ticket.subject}</td><td className="px-4 py-3">{ticket.libraries?.name ?? "General"}</td><td className="px-4 py-3">{supportCategoryLabels[ticket.category]}</td><td className="px-4 py-3">{supportPriorityLabels[ticket.priority]}</td><td className="px-4 py-3 font-bold">{supportStatusLabels[ticket.status]}</td><td className="px-4 py-3">{formatDate(ticket.created_at)}</td><td className="px-4 py-3">{formatDate(ticket.updated_at)}</td><td className="px-4 py-3"><button type="button" onClick={() => void openTicket(ticket)} className="rounded-xl bg-ulv-yellow px-3 py-2 text-xs font-black text-ulv-blue">Ver detalle</button></td></tr>)}</tbody></table></div></Card> : null}
